# TOROIDAL MASTER v0.2

**Successor working reference · September 6, 2026**
...
**Lineage and scope:** this is a separately versioned successor to `TOROIDAL MASTER v0.1` (SHA-256 `5d28790c6132a3dc5dfdb8036287b25e32c9c5b6acc767cf6112770001064359`). It preserves that baseline and adds only the five closure corrections specified in §8 of *Review of the four additional documents*. The acoustic-resonator findings and mythology-source material remain outside this master. The analytical additions below do not constitute software execution or numerical sampler validation.
...
### 6.4 Witness-relative non-descent on attained labels

Let `pi: X -> Q` be a projection and `w: X -> Y` a declared witness. Define the witness-relative non-descent set only on labels that are actually attained:

$$N_w(\pi)=\{q\in\pi(X):\exists x,y\in\pi^{-1}(q),\ w(x)\ne w(y)\}.$$

Then `N_w(pi)` is empty exactly when there is a unique set map `bar w: pi(X) -> Y` satisfying

$$w=\bar w\circ\pi.$$

Indeed, `bar w(q)` can be defined using any member of the fiber precisely when `w` is constant on that fiber. Restricting the codomain to `pi(X)` avoids irrelevant empty-fiber extension questions. This is a set-theoretic factorization statement; continuous or measurable factorization requires the corresponding additional assumptions.

For stochastic closure, the witness is the whole conditional law of the next projected state, not one realized successor or one scalar average. For a countable full-state model and finite Q, use

$$w(x)=\bigl(P_\pi(x,q')\bigr)_{q'\in Q},\qquad
P_\pi(x,q')=\sum_{z:\pi(z)=q'}P(x,z).$$

For a continuous full-state model, replace the point sum with the kernel probability `P(x,pi^-1(B))` for each measurable projected block B.

### 6.5 Proof status, sampled coverage, and boundaries are separate

The exact complement `Q minus N_w(pi)` is not the set of fibers that happened to be covered by a scan. Keep the mathematical claim separate from the evidence record:

| Evidence status | Required meaning |
|---|---|
| PROVED_CLEAN | Witness constancy is proved or exhaustively checked on the declared fiber. |
| LOCATED_FAILURE | A valid same-fiber, unequal-witness pair is established. |
| SAMPLE_CONSISTENT | Tested observations meet a declared statistical rule; universal constancy is not established. |
| UNKNOWN_OR_UNTESTED | Coverage, eligibility, or precision is insufficient. |

Zero observed failures in zero eligible comparisons is `UNKNOWN_OR_UNTESTED`, not clean. A finite noisy test also cannot establish exact equality of continuous probability parameters merely by accepting a nonzero equivalence margin.

Topological and parameter boundaries require their own declared domains. No topology is implicit in `N_w(pi)`. With the natural discrete topology on the eight parity labels, every subset is open and closed, so its topological boundary is empty. In a general topological space, `Q minus N`, `N`, and `boundary(N)` are not automatically a disjoint partition because the boundary can overlap N and its complement. A topological partition, when intended, is `interior(N)`, `boundary(N)`, and `interior(Q minus N)` after the topology is specified.

For a parameter-dependent study, define `N_lambda` for a declared family of kernels and put a topology on the parameter domain; a boundary in that domain then has a defined meaning. An explicitly defined edge boundary in a finite sector graph is another legitimate object, but it is not a topological boundary and does not by itself establish a physical bifurcation.

### 6.6 One-step witness retention is weaker than a closed replacement state

The refinement

$$\pi'(x)=(\pi(x),w(x))$$

always retains the chosen witness. Among set maps retaining both `pi` and `w`, it is the coarsest such record. It need not be a practical compression, and it does not automatically make `pi'` evolve autonomously.

A finite deterministic counterexample is:
...
The refined record predicts the next original label everywhere. Nevertheless x and y share refined label `(0,0)` while their next refined labels differ, so the refined state is not closed.

For a finite deterministic system, obtain the coarsest closed refinement by repeatedly splitting blocks according to successor blocks until the partition stabilizes. For a finite Markov system, split according to the vectors of transition probabilities into the current refined blocks and repeat until stable. Finite termination or a small sufficient state is not guaranteed for infinite systems. The restricted `(q,S_x,S_y,S_z)` result in §6.2 is stronger because its own update law was derived for the declared sector-only schedule.

### 6.7 Total-variation diameter and unavoidable reduced-law error

For each attained fiber, define its next-law diameter in total variation:

$$D_\pi(q)=\sup_{x,y\in\pi^{-1}(q)}
\frac12\sum_{q'\in Q}|P_\pi(x,q')-P_\pi(y,q')|.$$

Strong lumpability holds on that fiber exactly when `D_pi(q)=0`. For any single reduced prediction `K(q,.)`, the triangle inequality gives

$$\sup_{x\in\pi^{-1}(q)}
\operatorname{TV}(P_\pi(x,\cdot),K(q,\cdot))\ge\frac12D_\pi(q).$$

Thus a same-fiber pair provides both a closure counterexample and a lower bound on the error forced upon any one-law replacement. It supplies a lower bound on the diameter, not necessarily its full value.

At `L=2, J=1`, the two elementary Villain states in §6.1 have toggle probabilities `0.3678794412` and `0.5033689735`. Their distance is `0.1354895323`, so any single toggle prediction for both states must be wrong by at least `0.0677447662` for one of them: about 6.77 percentage points for that conditional elementary move. These are exact-law calculations evaluated numerically, not estimates from a run. Empirical diameter estimates require uncertainty and coverage controls; a small sampled diameter does not bound unobserved fibers without additional assumptions.

### 6.8 Parity non-descent for the complete Villain FH-REF-1 law

The elementary failure extends to the complete lazy random-scan Villain kernel specified in §7. A sector proposal occurs with probability `1/4` and then selects one of three axes, so for a given axis alpha,

$$P_{\mathrm{FH}}(q\oplus e_\alpha\mid x)=\frac1{12}p_\alpha(x).$$

For the family `x_m` with `M=0`, `I=2m Gamma_alpha`, `q=0`, and `a=L/(2J)>0`, compare `m=1` with `m=2`:

$$p_1=\frac{1+e^{-5a}}2,\qquad p_2=\frac{1+e^{-9a}}2,$$

and therefore

$$P_{\mathrm{FH}}(e_\alpha\mid x_1)-P_{\mathrm{FH}}(e_\alpha\mid x_2)
=\frac1{24}e^{-5a}(1-e^{-4a})>0.$$

Consequently, the complete Villain FH-REF-1 random-scan kernel is not strongly lumpable through q for finite `L>=2`, `J>0`, `0<t<1`, `h6=0`, and untruncated currents. Target invariance and full-state connectivity are compatible with this result: correct sampling does not require the observed parity process to be Markov.

For the `x_0,x_1` pair at `L=2, J=1`, the complete-kernel total-variation distance is `0.1354895323/12 = 0.0112907944`. Any single projected law therefore incurs error of at least `0.0056453972` on one member of this pair, about 0.565 percentage points per attempted step.

This result is confined to the specified Villain reference mixture. It does not resolve the older worm macro-schedule, constitute a software test, or automatically extend to cosine weights. In FULL-REF-1, q is fixed at zero, so the constant q projection closes trivially and carries no eight-sector dynamics.
...
| Witness-relative non-descent on attained labels | Incorporated in §6.4. |
| Proof status, sampled coverage, and boundary separation | Incorporated in §6.5. |
| One-step witness retention versus closed refinement | Incorporated in §6.6. |
| Total-variation diameter and unavoidable-error bound | Incorporated in §6.7. |
| Complete Villain FH-REF-1 parity non-descent | Incorporated analytically in §6.8; the older worm schedule and software validation remain separate. |
...
* [Review of the four additional documents](https://docs.google.com/document/d/14vGDM9PFPD0Qlh39WxwBZBWxizFsR8U2_vjWOkf2r6s)
...
The reference-sampler specification and the §6.4–§6.8 closure additions in this master have analytical arguments. They are not represented as numerical execution of the sampler. Acoustic-resonator corrections and mythology-source review remain in their separate records and are not incorporated here.
