...
Freeze status: Appendix B specification frozen; Appendix A §A.8 execution and A.10 artifact validation pending; execution hash blocked
...
Appendix B — Frozen Execution, Blinding, Size-Admissibility, and Provenance Card

Status: SPECIFICATION FROZEN. Appendix A §A.8 execution, A.10 artifact completion, the blind resource forecast, and the execution-root hash remain pending.

Scope: Operational freeze for Stages A–C. This appendix fixes the remaining representation labels, parameter and size inventory, finite-h_6 confirmation values, blinding boundary, statistical card, Q2 ratio and mixing implementation, confinement-radius upper-bound rule, resource gate, and provenance chain.

Controlling rule: The protocol body and Appendix A remain authoritative. This appendix may make their operational choices more specific but may not weaken an ensemble firewall, validation threshold, mixing requirement, size-admissibility rule, masking rule, or provenance dependency. If two instructions appear to conflict, the stricter gate controls. A scaffold, hash, or internally consistent pair of implementations is not empirical corroboration and is not a validation pass.

B.0 Gate state and witness separation

Q1, Q2, and Q3 remain distinct witnesses. Q1 or Q3 success cannot answer Q2; Q2 success cannot establish Q1 or Q3. Every output records model, representation, ensemble, parameter point, lattice size, boundary/holonomy convention, code commit, build manifest, chain bank, and seed derivation.

The execution root is forbidden until all of the following are true: Appendix B and the exact run inventory are hashed with no unresolved field; every mandatory A.8 gate has a passing immutable report; all eleven A.10 artifacts exist, hash successfully, and carry their required FROZEN or PASS state; the blind forecast passes B.9; and the no-selective-removal check passes. Appending this text does not open the gate.

B.1 Frozen ensemble and representation registry

1. C-DIR-PBC: direct cosine representation of Z_full. Authorized for Q1, bulk thermodynamics, the h_6=0 Q3 calibration, and primary finite-h_6 Q3 confirmation.

2. C-DUAL-FULL: cosine-current representation of Z_full. Authorized only where the direct/dual identity and boundary conventions have passed A.8. It does not expose an independently variable odd mod-two sector.

3. C-DUAL-FH-000: cosine-current representation of Z_FH=Z_000 with all three reference holonomies fixed trivial and q in Z_2^3 explicit. This is the primary Q2 production representation.

4. V-DUAL-FH-000: Villain fixed-holonomy representation used for exact tests, blind validation, and model-specific comparisons. Villain weights and couplings are never substituted for cosine weights or couplings.

5. V-CKT-AX-Z: reconstructed one-axis z-axial Villain comparator, including its periodic closure bit. It may validate the CKT mapping and benchmark only; it may not replace C-DUAL-FH-000, the canonical three-axis geometry, or any Q2 gate.

No observable, crossing value, amplitude, stationary sector probability, or verdict transfers between Z_full and Z_000 without an explicit proved identity or a separately reported cross-ensemble comparison. The numerical value of a locator coupling may define where a second ensemble is evaluated; all observables in that second ensemble are nevertheless estimated anew and retain its ensemble label.

B.2 Frozen points, sizes, and coupling grids

The three moderate gauge points are P1: kappa=0.800, t=tanh(kappa) approximately 0.664; P2: kappa=0.8673, t=0.700; and P3: kappa=1.000, t=tanh(kappa) approximately 0.762. P2 is the CKT Fig. 5 comparison point. Approximate displayed t values are labels; stored execution inputs contain full-precision kappa and computed t.

Stage B and Stage C use L in {12,16,24,32,48,64,96} at every P1–P3 point unless a pre-output formal amendment replaces the entire affected inventory. Stage B uses five J values for L<64 and three for L>=64. Stage C uses five J values for L<=32 and three for L>=48.

For each L, the blind locator writes J_L^(0) and a positive spacing delta J_L to the immutable inventory before any masked target channel is opened. The five-point grid is J_L^(0)+{-2,-1,0,1,2} delta J_L; the three-point grid is J_L^(0)+{-1,0,1} delta J_L. J_L^(0), delta J_L, proposal widths, measurement cadence, and production length may depend only on permitted calibration outputs and must be hashed in H_Inventory before target production. They may not be moved or extended because a target effect looks promising.

Q2 is evaluated at J_Q2(t)=1.01 J_c(t,0), where J_c(t,0) is the blinded Z_full locator output frozen before Q2 sector outputs are opened. C-DUAL-FH-000 production uses L in {48,64,96,128,160,192} at every P1–P3 point. The dual hard cap is L_max=192. A direct fixed-holonomy diagnostic may run through L=96 but cannot replace the dual production ladder or establish size admissibility by itself. The L=192 point is mandatory and cannot be dropped, deferred, relabeled optional, or silently shortened to satisfy a budget or improve a result.

B.3 Frozen Q3 finite-h_6 confirmation

The direct cosine confirmation set is h_6 in {-0.005,-0.0025,0,+0.0025,+0.005} on the Stage C inventory. The h_6=0 zero-field response

D_6(L)=d A_3^Phi/d h_6 at h_6=0 = Cov_0(A_3^Phi,H_6), with H_6=sum_i cos(6 theta_i),

is primary. The symmetric finite-field slopes

D_6^(0.0025)(L)=[A_3^Phi(+0.0025)-A_3^Phi(-0.0025)]/0.005

and

D_6^(0.005)(L)=[A_3^Phi(+0.005)-A_3^Phi(-0.005)]/0.010

are mandatory confirmation checks. For each L and point, A_3^Phi(+h)+A_3^Phi(-h) must be compatible with zero within three combined standard errors for both magnitudes; both symmetric slopes must agree with D_6(L) and with one another within three combined standard errors. Every sign and magnitude is retained. A failed sign-reversal or linearity check makes the finite-field confirmation unresolved at that point; the better-looking sign or magnitude may not be selected. The finite-h_6 dual charge-six branch remains secondary and unauthorized for production until its source-sector kernel has its own complete validation record.

B.4 Blind-validation boundary

Blind pilots may expose only calibration channels needed to choose proposal widths, locator centers and spacings, measurement cadence, autocorrelation windows, fixed production lengths, and resource forecasts. Q1 exponent/model preference, Q3 harmonic and response, and Q2 stationary sector probabilities, partition-ratio direction, and scientific verdict channels remain masked until H_Inventory, H_Pilot, H_Forecast, and all applicable validation reports are committed.

The validation custodian may compute the A.8 pass/fail diagnostics required for algebra, exact enumeration, detailed balance, reachability, symmetry, translated cycles, sector mixing, and the CKT comparator. Before unmasking, the acquisition side receives only the gate state and non-target calibration/resource summaries. Pilot bytes are immutable evidence, not informal notes; a rerun is a new artifact linked by a formal reason and may not overwrite the original.

B.5 RNG, chains, stopping, and common statistics

The RNG is Philox4x32-10. Each 128-bit seed is the first 128 bits of SHA-256 over one UTF-8 canonical JSON object with lexicographically sorted keys and a final LF. Required keys are H_AppB, H_Inventory, stage_id, representation_id, model_id, point_id, L, h_6, chain_bank, chain_id, purpose, and replicate_id. No seed may be reused across a distinct key tuple.

There are eight independent chains per parameter point and chain bank. For Q2, the primary bank starts one chain in each q in Z_2^3 and a separate blind restart bank starts all eight chains in q=000. The primary bank must pass the complete A.8.6 thresholds; the restart bank must supply an independently adequate stationary estimate for the required three-combined-standard-error compatibility test. Neither bank may borrow samples from another parameter point or size.

Thermalization is max(10^4,100 tau_max^pilot) macro-sweeps. Production adaptation is prohibited. Autocorrelation uses the Gamma method with an automatic self-consistent window. Block length is ceil(20 max_O tau_int,O), with a minimum of 32 measurement intervals. Uncertainty uses 5,000 hierarchical chain/block bootstrap replicates, with full cross-observable covariance inside every replicate. Fits use correlated nonlinear least squares; covariance modes with lambda/lambda_max<10^-10 are discarded and the discarded fraction is reported. An admissible fit has at least four lattice sizes and four residual degrees of freedom. Bootstrap p_GOF>=0.05 is acceptable; p_GOF<0.01 is strong rejection. Delta AICc>=10 is strong model preference. The frozen L_min sequence is 12,16,24,32,48 where enough larger sizes remain, and every admissible window is reported.

Fixed acquisition targets are SE(R_xi)<=0.002, SE(U_4)<=0.002, relative SE(chi_2)<=0.005, and SE(A_3^Phi)<=0.003. Production lengths are derived from masked pilot variances, recorded in H_Inventory, and never extended because an unmasked effect looks promising. Optional stopping is prohibited.

B.6 Independent ratio, symmetry, translated-cycle, and mixing freeze

For each axis, point, and blind pilot size, the independent Z_{e_alpha}/Z_000 calculation uses two separately seeded bidirectional free-energy ladders. On the common configuration space obtained by the frozen seam map T_alpha, endpoint weights are w_0(x) and w_{e_alpha}(T_alpha x). The bridge is w_lambda(x)=w_0(x)^(1-lambda) w_{e_alpha}(T_alpha x)^lambda on lambda in {0,1/8,2/8,3/8,4/8,5/8,6/8,7/8,1}. The preregistered estimator is multistate Bennett acceptance ratio with full ladder covariance. Artifact A10-07 must encode the seam map, endpoint normalizations, overlap diagnostics, and exact-enumeration recovery before the estimator can pass.

The ratio estimate must agree with 1-2 f_odd^(alpha) within three combined standard errors for every axis and pilot point. At isotropic couplings the three axes require a global correlated symmetry-test p-value of at least 0.05. The full canonical cycle tuple is also translated by r_x=(floor(L/2),0,0), r_y=(0,floor(L/2),0), and r_z=(0,0,floor(L/2)); the canonical plus three translated placements require a correlated consistency-test p-value of at least 0.05 at every blind validation point. No cycle placement may be selected after results are known.

At every Q2 production point and size, each axis requires at least 100 effective completed 0->1->0 sector round trips in the aggregate of the eight primary chains; rank-normalized R-hat<1.01 and ESS>=1000 for 1[q_alpha=1]; reported sector autocorrelation, acceptance, residence times, and the full 8-by-8 q transition matrix; and stationary compatibility of the even-start and all-000-start banks within three combined standard errors. If a true sector is too rare, only the partition-ratio bound and stopping rule frozen before target outputs were opened may establish a bound. Otherwise the required label is Q2 UNRESOLVED — SECTOR MIXING NOT VALIDATED.

B.7 Frozen confinement-radius estimator and size admissibility

In Z_000 define r_1(L)=R_tilde_1(L)/sqrt(2) from the charge-one correlator under the frozen geometry. Estimate its saturation radius on the Q2 ladder with both preregistered tail families:

S-exp: r_1(L)=xi_conf-a exp(-L/lambda), with a>=0 and lambda>0;

S-power: r_1(L)=xi_conf-b L^(-omega), with b>=0 and 0.5<=omega<=3.

Use correlated fits and the common hierarchical bootstrap on the frozen windows L_min in {48,64,96}; each admitted window has at least four sizes, p_GOF>=0.05, and no significant monotone trend left in its largest three residuals at the 0.05 level. xi_conf^UCB is the maximum one-sided 95% bootstrap upper limit over every admitted family and window. If no family/window is admitted, set xi_conf^UCB=+infinity for the decision rule.

Q2 is size-admissible only if L_max>=4 xi_conf^UCB. With L_max=192 this requires xi_conf^UCB<=48. Failure, equality uncertainty crossing 48, or an infinite bound requires the exact label Q2 INCONCLUSIVE — QUASI-DECONFINED WINDOW NOT EXCLUDED. No deeper coupling, smaller radius estimate, selected fit family, or selected window may be substituted after seeing the result.

B.8 CKT comparator freeze

At the Villain benchmark t=0.700, compare V-CKT-AX-Z with the canonical V-DUAL-FH-000 construction. The comparator must reproduce J_c=0.3335(3) within the published uncertainty and rho_s approximately 0.038 at J=0.336 within the declared simulation uncertainty; verify every A.7 axial mapping identity; and report all differences between one-axis and three-axis sector statistics. Agreement validates the implementation path only. Disagreement blocks the comparator gate and never authorizes changing the canonical ensemble, cosine coupling, geometry, or Q2 rule.

B.9 Resource forecast and immutable inventory

The hard total ceiling is C_max=20,000 physical CPU core-hours. Execution may begin only if the blind-pilot forecast satisfies C_forecast<=14,000 physical CPU core-hours, leaving at least 6,000 core-hours of reserve. The forecast must reconcile exactly, row for row, with the hashed run inventory: stage, model, representation, ensemble, point, L, J, h_6, chain bank, chain count, thermalization, production length, measurement cadence, and expected cost. A nominal estimate is not a pass.

If the complete frozen inventory does not fit the forecast gate, no expensive point—especially Q2 L=192—may be removed, deferred, made optional, or shortened after any validation or forecast output is opened. The only repairs are a dated formal amendment with a new hash and all affected revalidation plus a revised blind forecast, or a raised ceiling with a new hash.

B.10 A.10 artifact filenames and states

The companion scaffold contains exactly these eleven files:

1. 01_appendix_a_canonical_text.md
2. 02_frozen_geometry_specification.md
3. 03_transition_acceptance_table.md
4. 04_algebra_invariant_test_report.md
5. 05_small_volume_enumeration_and_tail_certificate.md
6. 06_eight_sector_transform_table.md
7. 07_partition_ratio_validation_report.md
8. 08_detailed_balance_reachability_report.md
9. 09_blind_mixing_roundtrip_pilot_report.md
10. 10_ckt_axial_mapping_comparator_report.md
11. 11_source_commit_build_environment_manifest.md

Files 01–03 and 11 must reach FROZEN with no PENDING execution field. Files 04–10 must reach PASS with machine-readable evidence references and content hashes. NOT RUN, PENDING, INCONCLUSIVE, UNRESOLVED, FAIL, a missing file, or a hash mismatch keeps H_Execution forbidden. The scaffold is a specification container only and is not evidence that any test ran or passed.

B.11 Hash chain, amendment rule, and current gate

Artifact content hashes use SHA-256 over exact stored bytes. Text artifacts are UTF-8 with LF line endings and a final LF; structured JSON uses lexicographically sorted keys, no insignificant whitespace, UTF-8 encoding, and a final LF. Each manifest records filename, byte count, content hash, generating command or executable, source commit, build/container digest, parent hashes, and completion state.

The lineage is H_Theory -> H_Protocol -> H_AppA -> H_AppB -> H_AppBAmend -> H_Inventory -> H_Pilot -> H_Forecast -> H_Execution. An absent amendment is represented by the canonical null-amendment record, not by omitting the node. Pilot and forecast files are immutable children of the exact inventory they evaluated.

After any validation, pilot, forecast, or target output is opened, no axis, cycle placement, lattice size, parameter point, kernel, chain bank, or failed check may be silently removed. Any change requires a dated formal amendment, a new hash, revalidation of every affected item, a revised blind resource forecast, and a new descendant chain. Agreement between implementations, models, or fit families is a consistency check, not independent empirical provenance.

Current Appendix B specification state: FROZEN. Current A.8/A.10 execution state: NOT RUN/PENDING. Current execution-root state: BLOCKED.

\boxed{\text{EXECUTION HASH REMAINS BLOCKED UNTIL EVERY A.8 AND APPENDIX B GATE PASSES.}}
